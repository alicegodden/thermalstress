...
for chrom, pos, gene in genes_data:
    ...
    ax.annotate(gene, xy=(x, y), ha='center', va='center', fontsize=11.5, fontweight ='bold',
             arrowprops=dict(arrowstyle= "<-", color='red', facecolor='none', #shrink=0.05,
                                                  linewidth=0))
...
plt.plot([], [], label='', color='none')  # Adjust the color as needed #plt.legend(loc='center left', bbox_to_anchor=(1, 0.5)) # to have a legend of the chromosome colors on the right
...
# save the file
output_file = 'FILTER_PY_WIN150_INT_femaleT_cen_genes.png'  # Replace with your desired filename and extension
